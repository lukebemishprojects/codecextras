...
class DispatchScreenEntryProvider<K, T> implements ScreenEntryProvider {
    ...
    public DispatchScreenEntryProvider(EntryCreationInfo<K> keyInfo, JsonElement jsonValue, String key, Consumer<JsonElement> update, EntryCreationContext context, Map<K, DataResult<ConfigScreenEntry<? extends T>>> entries) {
        ...
        for (var entry : entries.entrySet()) {
            var keyResult = keyInfo.codec().encodeStart(context.ops(), entry.getKey());
    ...
    public void addEntries(ScreenEntryList list, Runnable rebuild, Screen parent) {
        ...
        var contents = Button.builder(Component.literal(keyValue == null ? "" : keyValue), b -> {
            Minecraft.getInstance().setScreen(new ChoiceScreen(parent, keyInfo.componentInfo().title(), keys, keyValue, newKeyValue -> {

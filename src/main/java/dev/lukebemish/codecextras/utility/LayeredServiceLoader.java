...
public final class LayeredServiceLoader<T> {
    ...
    private final ClassValue<SingleImplementation<T>> providersValue = new ClassValue<>() {
        ...
        protected SingleImplementation<T> computeValue(Class<?> type) {
            ...
            if (type.getModule().getLayer() == null) {
                ServiceLoader.load(service, type.getClassLoader()).forEach(provider -> singleImplementation.implementations.put(provider.getClass(), provider));
            } else {
                ServiceLoader.load(type.getModule().getLayer(), service).forEach(provider -> singleImplementation.implementations.put(provider.getClass(), provider));
            }
    ...
    public static final class SingleImplementation<T> {
        private final SequencedMap<Class<?>, T> implementations = new LinkedHashMap<>();
    }
}
